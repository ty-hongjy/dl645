...
支持省略第二个68的帧结构，生成和实际一致的命令帧，适配绝大多数DL645-1997电表。
...
### 测试案例：
97-test.ts 测试案例，生成并发送命令帧并打印出命令帧。
97-test-s.ts 测试案例，生成并发送命令帧通过485串口到电表,适配真实电表地址000048604296生成正确命令，用485串口调试工具可以查看命令帧。
